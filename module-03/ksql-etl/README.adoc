...
ksql> CREATE STREAM stationsfeedwithkey WITH (KAFKA_TOPIC='STATIONSFEEDWITHKEY', VALUE_FORMAT='AVRO') \
...
...
2 | -7.650282859802246 | -8.648320198059082 | 998
2 | -7.650282859802246 | -8.648320198059082 | 999
2 | -7.650282859802246 | -8.648320198059082 | 1000
...
3 | 4.619408130645752 | 3.621914863586426 | 998
3 | 4.619408130645752 | 3.621914863586426 | 999
...
+
```sql
ksql> CREATE TABLE extreme_temperatures WITH (KAFKA_TOPIC='EXTREME_TEMPERATURES', VALUE_FORMAT='AVRO') \
AS \
SELECT station_id, \
    MAX(temperature) as max_temp, \
    MIN(temperature) min_temp, \
...
```
+
It should give the same output as above...
...
$ docker-compose exec connect \
    ...
    "config": {
        ...
        "tasks.max": "1",
        "table.name.format": "${topic}",
        "fields.whitelist": "station_id,max_temp,min_temp,count",
        "pk.mode": "record_value",
